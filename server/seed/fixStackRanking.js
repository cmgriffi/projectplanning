...
// Fix stack ranking for ideas
const fixStackRanking = async () => {
  try {
    // Get all ideas
    ...
    for (const [businessFunction, group] of Object.entries(businessFunctionGroups)) {
      ...
      // Update order for each idea in the group
      for (let i = 0; i < group.length; i++) {
        const idea = group[i];
        await Idea.findByIdAndUpdate(idea._id, { order: i });
        totalUpdated++;
      }
    ...
    // Verify the update
    const updatedIdeas = await Idea.find({}).sort({ businessFunction: 1, order: 1 });
    ...
    for (const idea of updatedIdeas) {
      ...
      if (count < 3) { // Show only first 3 ideas for each business function
        console.log(`  ID: ${idea._id}, Title: ${idea.title}, Order: ${idea.order}`);
      }
    ...
    console.error('Error fixing stack ranking:', error);
    return false;
  }
};
...
const main = async () => {
  ...
  // Fix stack ranking
  const fixed = await fixStackRanking();
  if (!fixed) {
    console.error('Failed to fix stack ranking');
    process.exit(1);
  }
